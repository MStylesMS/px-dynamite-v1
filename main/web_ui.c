...
static char s_prop_id[32] = "Dynamite32Prop";
...
static esp_err_t save_connection_cfg_nvs(void)
{
    ...
    dynamite_engine_get_config_json(prop_cfg, sizeof(prop_cfg));
    if (!conn_cfg_lock()) {
        return ESP_ERR_TIMEOUT;
    }
    cfg_copy = s_conn_cfg;
...
static void sanitize_network_name(const char *src, char *out, size_t out_size)
{
    ...
    if (w == 0) {
        strncpy(out, "px-dynamite-v1", out_size - 1);
        out[out_size - 1] = '\0';
        return;
    }
...
static void build_default_identity(void)
{
    ...
    if (err == ESP_OK) {
        snprintf(s_prop_id, sizeof(s_prop_id), "Dynamite32Prop");
    }
    ...
    if (s_conn_cfg.network_name[0] == '\0') {
        ...
        if (esp_read_mac(mac, ESP_MAC_WIFI_STA) == ESP_OK) {
            snprintf(network_id, sizeof(network_id), "px-dynamite-v1-%02x%02x", mac[4], mac[5]);
        } else {
            copy_bounded_local(network_id, sizeof(network_id), "px-dynamite-v1");
        }
...
static const char *dynamite_led_hint_name_local(dynamite_led_hint_t hint);
...
static int mqtt_state_interval_ms(void)
{
    ...
    dynamite_engine_get_config_json(cfg, sizeof(cfg));
...
static void mqtt_publish_state(bool retained)
{
    ...
    dynamite_engine_get_mqtt_status_json(status_json, sizeof(status_json));
    mqtt_publish_or_warn(topic, status_json, 2, retained);
}
...
static bool mqtt_publish_engine_events(bool publish_state_after)
{
    ...
    mqtt_build_topic(topic, sizeof(topic), "state");
    while (dynamite_engine_pop_event_json(event_json, sizeof(event_json))) {
...
static void mqtt_publish_legacy_heartbeat(void)
{
    ...
    /* GM watches /Paradox/Props and /Paradox/ParadoxDynamiteProp/Props. */
    snprintf(payload,
             sizeof(payload),
    ...
    {
        char extra[160];
        mqtt_build_topic(extra, sizeof(extra), "Props");
        if (strcmp(extra, topic) != 0) {
            mqtt_publish_or_warn(extra, payload, 0, false);
        }
    }
}
...
static void mqtt_publish_announce(void)
{
    ...
    dynamite_battery_snapshot_t battery_snap;
    esp_netif_t *sta_netif = esp_netif_get_handle_from_ifkey("WIFI_STA_DEF");
    const esp_app_desc_t *app = esp_app_get_description();
    ...
    mqtt_build_topic_locked(commands_topic, sizeof(commands_topic), "command");
    conn_cfg_unlock();
    ...
    dynamite_engine_get_battery_snapshot(&battery_snap);
    ...
    snprintf(announce,
             ...
             dynamite_led_hint_name_local(dynamite_engine_get_led_hint()),
             app->version,
             app->version,
             app->date,
             app->time,
...
static void mqtt_apply_follower_payload(const char *payload, int payload_len)
{
    ...
    dynamite_engine_get_state_json(local_state_json, sizeof(local_state_json));
    ...
    if (local_remaining_s >= 0 &&
        ...
        if (dynamite_engine_handle_command_json(cmd, response, sizeof(response)) == ESP_OK) {
    ...
    if (has_pause && game_paused) {
        if (strcmp(local_state, "countdown") == 0) {
            if (dynamite_engine_handle_command_json("{\"command\":\"pause\"}", response, sizeof(response)) == ESP_OK) {
        ...
        if (dynamite_engine_handle_command_json("{\"command\":\"resume\"}", response, sizeof(response)) == ESP_OK) {
            mqtt_publish_follow_event("commandOverridden",
                                      local_remaining_s,
                                      remote_remaining_s,
...
static void mqtt_handle_command_payload(const char *payload, int payload_len)
{
    ...
    if (dynamite_engine_handle_command_json(cmd_json, response, sizeof(response)) == ESP_OK) {
        /* Do not publish HTTP-style ACK JSON onto CF /state. */
...
static void mqtt_on_connected(void *ctx)
{
    ...
    mqtt_build_topic(commands_topic, sizeof(commands_topic), "command");
...
static void mqtt_state_task(void *arg)
{
    ...
    while (true) {
        ...
        while (dynamite_engine_pop_warning_message(warning_msg, sizeof(warning_msg))) {
...
static esp_err_t state_get_handler(httpd_req_t *req)
{
    ...
    payload = (char *)malloc(DYN_STATE_JSON_MAX);
    if (!payload) {
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Out of memory");
        return ESP_ERR_NO_MEM;
    }

    dynamite_engine_get_state_json(payload, DYN_STATE_JSON_MAX);
    httpd_resp_set_type(req, "application/json");
...
static void build_unified_config_json(char *out, size_t out_size)
{
    ...
    dynamite_engine_get_config_json(prop_payload, sizeof(prop_payload));

    mqtt_build_topic(commands_topic, sizeof(commands_topic), "command");
    mqtt_build_topic(state_topic, sizeof(state_topic), "state");
    mqtt_build_topic(events_topic, sizeof(events_topic), "state");
...
static esp_err_t config_defaults_get_handler(httpd_req_t *req)
{
    ...
    dynamite_engine_get_default_config_json(payload, sizeof(payload));
    httpd_resp_set_type(req, "application/json");
...
static esp_err_t command_post_handler(httpd_req_t *req)
{
    ...
    engine_err = dynamite_engine_handle_command_json(body, response, sizeof(response));
...
static esp_err_t config_post_handler(httpd_req_t *req)
{
    ...
    engine_err = dynamite_engine_apply_config_json(body, persist, response, sizeof(response));
    if (engine_err == ESP_ERR_TIMEOUT) {
        cJSON_Delete(root);
        free(body);
        return send_json_error(req, "503 Service Unavailable", "busy");
...
static esp_err_t config_restore_post_handler(httpd_req_t *req)
{
    ...
    engine_err = dynamite_engine_restore_defaults(false, response, sizeof(response));
    if (engine_err == ESP_ERR_TIMEOUT) {
        return send_json_error(req, "503 Service Unavailable", "busy");
    }
    if (engine_err != ESP_OK) {
...
static esp_err_t connection_get_handler(httpd_req_t *req)
{
    ...
    char ap_ssid[33] = "Paradox-PXDynamiteV1";
    connection_cfg_t cfg_snap;
...
static esp_err_t ws_handler(httpd_req_t *req)
{
    ...
    if (payload[0] == '\0') {
        ...
        esp_err_t engine_err = dynamite_engine_handle_command_json(payload, reply_buf, sizeof(reply_buf));
        if (engine_err == ESP_ERR_TIMEOUT) {
...
static void wifi_on_sta_connected(const char *ip_text, void *ctx)
{
    ...
    /* Notify dynamite engine of WiFi connection for state snapshot RSSI/SSID. */
    svc_wifi_get_status(&wifi_status);
    if (wifi_status.connected) {
        dynamite_engine_notify_wifi_connected(wifi_status.rssi);
        dynamite_engine_set_wifi_ssid(wifi_status.ssid);
    }
...
static void wifi_on_sta_disconnected(int reason, const char *reason_text, void *ctx)
{
    ...
    /* Notify dynamite engine of WiFi disconnection. */
    dynamite_engine_notify_wifi_disconnected();
...
esp_err_t web_ui_start(void)
{
...

    {
        ...
        snprintf(ap_ssid, sizeof(ap_ssid), "Paradox-PXDynamiteV1-%02X%02X", mac[4], mac[5]);
